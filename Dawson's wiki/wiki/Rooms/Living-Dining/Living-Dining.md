---
name: Living-Dining
confidence: medium
source: "raw/drive/jm-house-moodboard-2026-06-07.md; raw/drive/jm-house-moodboard-v2-2026-06-07.md; raw/sheets/reno-sheet-bud-studio-2026-06-07.md"
---

# Living-Dining

Combined Living Room + Dining Room — open-plan per the moodboard's stated preference. The
furniture tracker, the Bud Studio scope, and the floor plan ("LIVING DINING" as one zone) all
treat it as a single space, so it's compiled here as one room.

## Design principles (from the J&M House Moodboard)

- Question whether a sofa is even needed (family spends more time at the dining table)
- Big dining table integrated into the living space, ideally behind/near the sofa
- All blinds: top-mounted, floor-length, end-to-end (bigger than window OK)
- Dedicated coffee corner
- TV wall kept clean/minimal — "just TV and TV console in similar colour to dining"
- Two-tone wall, floor-to-ceiling curtains, light grey carpet, dark greyish-brown extendable
  dining table with matching chairs

## Bud Studio Appendix A scope for this area

- False-ceiling/partition wall at TV wall incl. timber TV support — $725 (Section E item 2)
- Seated TV console + settee w/ storage — $1,710 + $2,235 (Section J items 3–4)
- DB-box & dry-pantry cabinetry at Entrance Foyer — Section J items 1–2a

## Items

- [[Dining Table]] — ordered, $1,450.00 (Other Furniture)
- [[TV]] — ordered, $3,199.00 (Gain City — Samsung 65" The Frame)
- [[Sound Bar]] — shortlisted, $899.00 (Sonos Beam Gen 2)
- [[TV Console - Storage (IKEA EKET)]] — shortlisted, $260.00 (possible overlap w/ custom carpentry — see note)
- [[Coffee Machine]] — shortlisted, $899.00 (Nespresso Creatista Plus)
- [[Table Lamps (Kave)]] — shortlisted, $460.00 combined (Kave Malachi + Nuvira)

## Open follow-ups

- [[TASK-0007 - Track Other Furniture dining table — balance payment & delivery]]

## Sources

- `raw/drive/jm-house-moodboard-2026-06-07.md` (slides 9, 13–15)
- `raw/drive/jm-house-moodboard-v2-2026-06-07.md` (slides 5–10)
- `raw/sheets/reno-sheet-bud-studio-2026-06-07.md` (Furniture & Appliances tracker)
- `raw/drive/skyville-dawson-bud-studio-first-draft-comments-2026-06-07.md` (pages 3–9)

---
*Compiled 2026-06-08 from approved review-queue proposal `system/review_queue/2026-06-07-living-dining-items.md`.*
